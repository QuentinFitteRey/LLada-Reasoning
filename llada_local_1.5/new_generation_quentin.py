...
def get_transfer_index_with_gumbel_noise(
    logits,
    temperature,
    remasking,
    mask_index,
    ...
    """
    Applies repetition penalty, injects Gumbel noise, selects new tokens,
    and determines which indices to update.
    """
    # 1) Repetition penalty (as before)
    if repetition_penalty > 1.0 and penalty_context is not None:
        for i in range(logits.shape[0]):
            gen_slice = penalty_context[i, prompt_len:]
            if mask_id is not None:
                gen_slice = gen_slice[gen_slice != mask_id]
            uniq = torch.unique(gen_slice)
            if uniq.numel() > 0:
                logits[i, :, uniq] /= repetition_penalty

    # 2) Inject Gumbel noise to the logits for sampling diversity
    logits_noisy = add_gumbel_noise(logits, temperature=temperature)

    # 3) Sample new tokens via argmax over the *noisy* logits
    x0 = torch.argmax(logits_noisy, dim=-1)

    # 4) Compute confidence (still on the *original* logits)
    if remasking == 'low_confidence':
    ...
    # 5) Mask out anything outside your masked positions
    x0 = torch.where(mask_index, x0, x)
    confidence = torch.where(mask_index, x0_p, -np.inf)

    # 6) Pick top‑k tokens by confidence
    transfer_index = torch.zeros_like(x0, dtype=torch.bool)
    if threshold is not None:
        # if thresholding, cap transfers to at most mask count
        num_transfer_tokens = mask_index.sum(dim=1, keepdim=True)

    for b in range(confidence.shape[0]):
        vals, idxs = torch.topk(confidence[b], k=num_transfer_tokens[b])
        # optionally enforce threshold
        if threshold is not None:
