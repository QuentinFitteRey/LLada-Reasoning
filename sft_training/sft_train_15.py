...
def main():
    ...
    parser.add_argument('--pretrained-model', type=str, default="./llada_local_1.5", help="Path to the base Hugging Face model.")
    parser.add_argument('--sft-data', type=str, default='./sft_data/combined', help='Path to SFT DatasetDict.')
    parser.add_argument('--output-dir', type=str, default='./checkpoints/llada_sft_15_FINAL', help='Directory to save checkpoints and logs.')
    ...
    parser.add_argument('--sft-lora-weights', type=str, default="/home/hice1/qfitterey3/scratch/LLada-Reasoning/checkpoints/llada_sft_15_FINAL/step-500/sft_adapter", help="Optional: Path to trainable LoRA weights from a previous SFT run to continue training.")
    parser.add_argument('--resume-from-checkpoint', type=str, default="/home/hice1/qfitterey3/scratch/LLada-Reasoning/checkpoints/llada_sft_15_FINAL/step-500", help="Path to resume trainer state (optimizer, scheduler, step).")
    ...
    # MODIFICATION: Initialize resume_local_step to -1 (no resume)
    resume_local_step = -1
    ...
    if state:
        ...
        # MODIFICATION: Load the local step to know which batch to resume from
        resume_local_step = state.get("local_step", -1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # MODIFICATION: Logic to skip batches if resuming mid-epoch
        start_step_for_epoch = 0
        if resume_local_step != -1:
            if is_main: print(f"Resuming epoch {epoch} by skipping to batch {resume_local_step + 1}")
            start_step_for_epoch = resume_local_step + 1
            # IMPORTANT: Reset so we don't skip in the next epoch
            resume_local_step = -1

        for step, batch in enumerate(train_loader):
            # MODIFICATION: Skip batches that have already been processed in a previous run
            if step < start_step_for_epoch:
                continue
            ...
            if (step + 1) % accum_steps == 0:
                ...
                if global_step > 0 and global_step % args.checkpoint_interval == 0:
                    ...
                    if is_main:
                        ...
                        # MODIFICATION: Add the local_step (batch number) to the checkpoint
                        state = {
                            "optimizer": optimizer.state_dict(), 
                            "scheduler": scheduler.state_dict(), 
                            "global_step": global_step, 
                            "epoch": epoch, 
                            "local_step": step, # Save the current batch number
