    ...
    def attention(
        ...
        if layer_past is not None: 
            ...
            if replace_position is None:
                ...
                # Make sure to convert any bf16 tensors to float32 before the operation
                if past_key.dtype == torch.bfloat16:
                    past_key = past_key.float()
                    past_value = past_value.float()
        ...
        if self.config.rope:
            ...
            # YarnRoPE wants seq_len_override: Optional[int]
            if replace_position is None:
                q, k = self.rotary_emb(q, k)
            else:
