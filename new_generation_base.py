...
def main():
    ...
    time_start.record()
    out = generate_with_dual_cache(model, input_ids, steps=128, gen_length=128, block_length=32, temperature=0.0, remasking='low_confidence')
    time_end.record()
    torch.cuda.synchronize()  # Wait for the events to be recorded
    elapsed_time = time_start.elapsed_time(time_end)  # Time in milliseconds
    print(f"Generation time: {elapsed_time / 1000:.2f} seconds")
