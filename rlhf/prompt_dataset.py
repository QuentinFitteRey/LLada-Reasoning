...
class PromptDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        # 2. NEW STEP: Apply the chat template to each prompt
        # This formats the prompt exactly as the model expects for generation.
        formatted_prompts = []
        for p in prompt_list:
            # This structure is based on your previous map_prompt function
            messages = [{"role": "user", "content": p.strip()}]
            formatted_p = self.tokenizer.apply_chat_template(
                messages,
                add_generation_prompt=True,
                tokenize=False  # We get the formatted string back
            )
        ...
        # 4. Calculate the length of each formatted prompt
