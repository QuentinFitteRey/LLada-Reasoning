...
def calc_sft_loss(model, batch, mask_id, pad_id, eps, think_end_id, answer_weight):
    ...
    # --- Start of New Weighting Logic ---
    # Find tokens that come after the first '</think>' token
    # The cumulative sum will be 0 for all tokens up to and including the first '</think>', and >0 after.
    is_answer_token_mask = torch.cumsum((ids == think_end_id), dim=1) > 0

    # Create weights tensor, default to 1.0
    weights = torch.ones(b, l, device=dev)
    # Apply the answer_weight to the identified answer tokens
    weights[is_answer_token_mask] = answer_weight
    # --- End of New Weighting Logic ---
    ...
    # Sum the weighted loss and normalize by the number of answer tokens
...
def run_validation(model, loader, dev, mask_id, pad_id, eps, think_end_id, answer_weight, limit=None):
    ...
    for i, batch in enumerate(loader):
        ...
        # Pass new args to use the same dynamic logic as training
