...
# -------- Core Functions --------
# MODIFIED: forward_process now handles dynamic ratios
def forward_process(input_ids, mask_token_id, mask_ratio):
    b, l = input_ids.shape
    device = input_ids.device

    # Handle both a single float or a tensor of ratios per sequence
    if isinstance(mask_ratio, torch.Tensor):
        # Unsqueeze from [b] to [b, 1] to allow broadcasting
        p_mask = mask_ratio.to(device).unsqueeze(1).expand(b, l)
    else:
        # Original behavior for a single float
...
# MODIFIED: calc_sft_loss now uses eps-based dynamic masking
def calc_sft_loss(model, batch, mask_id, pad_id, eps):
    ids, p_lens = batch["input_ids"], batch["prompt_lengths"]
    dev, b, l = ids.device, ids.shape[0], ids.shape[1]
    if l == 0: return None, 0.0

    # Dynamic masking logic from pre-training
    ...
    # Crucially, do not mask the prompt tokens
    ...
    # Normalize by the probability of masking each token
...
# MODIFIED: run_validation now accepts eps and uses dynamic masking
@torch.no_grad()
def run_validation(model, loader, dev, mask_id, pad_id, eps, limit=None):
    model.eval()
    total_loss, count = 0.0, 0
    for i, batch in enumerate(loader):
        if limit and i >= limit: break
        batch = {k: v.to(dev) for k, v in batch.items()}
        # Pass eps to use the same dynamic logic as training
        loss_val = calc_sft_loss(model, batch, mask_id, pad_id, eps=eps)[1]
...
def main():
    ...
    if global_step == 0:
        ...
        # MODIFIED: Pass eps to validation
        initial_val_loss = run_validation(model, val_loader, device, mask_id, pad_id, eps=args.eps, limit=args.val_limit_batches)
        if is_main:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            # MODIFIED: Pass eps to loss calculation
            loss, log_loss = calc_sft_loss(model, batch, mask_id, pad_id, eps=args.eps)
            ...
            if (step + 1) % accum_steps == 0:
                ...
                if global_step > 0 and global_step % args.validation_interval == 0:
                    val_loss = run_validation(model, val_loader, device, mask_id, pad_id, eps=args.eps, limit=args.val_limit_batches)
                    if is_main:
